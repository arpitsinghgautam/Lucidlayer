...
def handle_event(event):
    ...
    if event_type == "message" and event.get("channel_type") == "im" and "subtype" not in event:
        ...
        if not user_msg:
            return
        prompt = (
            "You're a sassy, humorous, but helpful virtual assistant. Reply playfully to this user message, "
            "but still provide useful info if needed. Avoid being robotic. Think snarky but charming:\n\n" +
            f"User: {user_msg}"
        )
        try:
